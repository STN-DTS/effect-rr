...
flowchart LR
  subgraph Process["One Node process (server.ts)"]
    ...
    subgraph Hex["Effect application core (server/)"]
      ...
      RT["runtime.ts<br/>AppLayer → ManagedRuntime (one per process)"]
    end
  end
...
server/                               # Effect core. Knows nothing about React/React Router.
  ...
  runtime.ts                          # AppLayer, AppServices, makeRuntime(), loadServerConfig()
...
- **Application** (`test/application`): every use case runs against fake ports provided through test
  Layers, with `TestClock` for timestamps.
...
- **Bridge and routes** (`test/routes`): loaders and actions are called directly with a real
  `RouterContextProvider` and a real `ManagedRuntime` built from test layers. The tests assert statuses,
  redirects, DTOs, compile-time exhaustiveness of `onError`, and the middleware's request id.
